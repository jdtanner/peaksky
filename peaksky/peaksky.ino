...
//Below line disabled to trial using minimum possible memory.
...
void setup() {
  ...
  resistorDivider = 0.1304347826087;

  //Set up pin to enable radio, PWM and SD
  ...
  while (Serial.available() > 0) {
    ...
    if (checkNMEASentence > 0) {
      ...
      if (numberOfSatellites >= 1) {
        ...
        batteryVoltage = ((analogRead(A0)/1024.0)*1.1)/resistorDivider;
        ...
        dtostrf(batteryVoltage, 4, 2, batteryVoltageBuffer);
        ...
        //Write ransmit buffer to dataLog.txt
        //dataLog = SD.open("dataLog.txt", FILE_WRITE);
        //dataLog.print(transmitBuffer);
        //dataLog.close();
